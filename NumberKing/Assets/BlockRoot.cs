...
public class BlockRoot : MonoBehaviour {
    ...
    private static BlockRoot instance;
    ...
    public static BlockRoot Instance
    {
        get
        {
            if (instance == null)
            {
                instance = FindObjectOfType<BlockRoot>();
                if (instance == null)
                {
                    GameObject container = new GameObject("BlockRoot");
                    instance = container.AddComponent<BlockRoot>();
                }
            }
            return instance;
        }
    ...
    {
        Vector3 postion = new Vector3(-(Block.BLOCK_NUM_X / 2.0f - 0.5f),
            (Block.BLOCK_NUM_Y / 2.0f - 0.5f), 0.0f);

        postion.x += (float)i_pos.x * Block.COLLISION_SIZE;
        postion.y += (float)i_pos.y * Block.COLLISION_SIZE;
